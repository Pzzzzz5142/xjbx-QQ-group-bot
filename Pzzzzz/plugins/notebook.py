...
async def notebook(session: CommandSession):
    ...
    if session.is_first_run and args.add != None:
        ...
        try:
            values = await db.conn.fetch(f'''select noteind from quser where qid = {session.event.user_id};''')
        ...
        try:
            state = await db.conn.execute('''insert into notebook (qid,item,ind) values ({0},'{1}',{2});'''.format(session.event.user_id, args.add, ind))
        except asyncpg.exceptions.ForeignKeyViolationError as e:
            await db.conn.execute('''insert into quser (qid,swid) values ({0},{1});'''.format(session.event.user_id, swFormatter(session.event.sender['card'] if session.event['message_type'] != 'private' else '-1')))
            state = await db.conn.execute('''insert into notebook (qid,item,ind) values ({0},'{1}',{2});'''.format(session.event.user_id, args.add, ind))
        except asyncpg.exceptions.UniqueViolationError as e:
            session.finish('你已经添加过该物品啦！')
        await db.conn.execute(f'''update quser set noteind = {ind + 1} where qid={session.event.user_id}''')
        session.finish('物品：{0} 添加完毕！'.format(args.add))

    elif session.is_first_run and args.list == True:
        values = await db.conn.fetch('''select * from notebook where qid={0} order by ind;'''.format(session.event.user_id))
        if len(values) == 0:
        ...
        if session.is_first_run:
            ...
            if args.delt[0]=='*':
                ...
                values = await db.conn.fetch('''select * from notebook where qid={0} and ind = '{1}';'''.format(session.event.user_id, args.delt))
            else:
                values = await db.conn.fetch('''select * from notebook where qid={0} and item = '{1}';'''.format(session.event.user_id, args.delt))
            if len(values) == 0:
                session.finish('貌似，并没有找到该记录？你输入的记录为 {0}'.format(args.delt if flg==0 else f"序列号：{args.delt + 1}"))
            item=values[0]['item']
            session.state['del'] = item
            session.state['delind'] = values[0]['ind']
            session.pause(f'确定要删除「{item}」吗？（请输入「是」或「否」）')
        if stripped_arg == '是':
            state = await db.conn.execute('''delete from notebook where qid={0} and item = '{1}';
            update notebook set ind = ind - 1 where ind > {2} and qid={0};
            update quser set noteind = noteind - 1 where qid = {0};'''.format(session.event.user_id, session.state['del'], session.state['delind']))
            session.finish('确认删除「{}」惹！'.format(session.state['del']))
        else:
            session.finish('删除撤销！')
