...
async def notebook(session: CommandSession):
    ...
    if session.is_first_run and args.add != None:
        ...
        async with db.pool.acquire() as conn:
            try:
                values = await conn.fetch(f'''select noteind from quser where qid = {session.event.user_id};''')
        ...
        async with db.pool.acquire() as conn:
            values = await conn.fetch('''select * from notebook where qid={0} order by ind;'''.format(session.event.user_id))
        if len(values) == 0:
        ...
        async with db.pool.acquire() as conn:
            ...
            if stripped_arg == '是':
                state = await conn.execute('''delete from notebook where qid={0} and item = '{1}';
